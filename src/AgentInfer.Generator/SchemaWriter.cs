...
internal static class SchemaWriter
{
    ...
    /// </para>
    /// <para>
    /// AgentInfer's attributes and DataAnnotations are both read. Declaring both
    /// on one property is AIN015 rather than a precedence rule, because a
    ...
    private static bool IsDoublyConstrained(IPropertySymbol property)
    {
        ...
        foreach (var attribute in property.GetAttributes())
        {
            ...
            {
                case "AgentInfer.BoundedAttribute":
                case "AgentInfer.SizedAttribute":
                    agentInfer = true;
                    break;
    ...
    internal static Bounds BoundsOf(IPropertySymbol property)
    {
        ...
        foreach (var attribute in property.GetAttributes())
        {
            ...
            switch (name)
            {
                case "AgentInfer.BoundedAttribute" when arguments.Length >= 2:
    ...
    /// unrecognized value. Naming it at build time is <c>AIN008</c>.
    /// </remarks>
    ...
    private static string? ScalarSchema(ITypeSymbol type)
    {
        if (type.TypeKind == TypeKind.Enum)
        {
            // A flags enum is a set, not a choice; AIN008 names it at build.
