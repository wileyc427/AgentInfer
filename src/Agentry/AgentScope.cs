...
/// </para>
/// <para>
/// The same discipline as <c>agentry.tool.calls_per_turn</c>, one level up.
...
public sealed class AgentScope : IDisposable
{
    ...
    public void Dispose()
    {
        ...
        _activity?.SetTag("agentry.workflow.operations", Operations);
        _activity?.SetTag("agentry.workflow.requests", Requests);
        _activity?.SetTag("agentry.workflow.tool_calls", ToolCalls);
        _activity?.Dispose();
    }
