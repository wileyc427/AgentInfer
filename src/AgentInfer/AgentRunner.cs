...
public sealed class AgentRunner(IChatClient client, ILogger<AgentRunner>? logger = null)
{
    ...
    /// not, and nothing here can tell which. Same reading AIN009 gives a prompt
    /// named twice.
    /// </summary>
    ...
    private void Record(AgentCall call, ToolCallLog log)
    {
        ...
        activity?.SetTag("agentinfer.tools.offered", log.Offered);
        activity?.SetTag("agentinfer.tool.calls", log.Invocations);
        activity?.SetTag("agentinfer.tool.denials", log.Denials);
